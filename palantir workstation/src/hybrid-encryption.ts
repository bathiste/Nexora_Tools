...
export interface EncryptedMessage {
    ...
    salt: Uint8Array;
    nonce: Uint8Array;
    epk: Uint8Array; // X25519 ephemeral public key
    kyberCt: Uint8Array; // Kyber768 ciphertext
    ciphertext: Uint8Array; // ChaCha20-Poly1305 ciphertext
}

export class HybridEncryption {
    private x25519KeyPair: CryptoKeyPair | null = null;
    private kyberKeyPair: KeyPair | null = null;

    // Generate receiver long-term keys
    async generateReceiverKeys(): Promise<{
        ...
        // X25519 key pair
        this.x25519KeyPair = await window.crypto.subtle.generateKey(
            { name: "X25519" },
            true,
        ...
        const x25519PublicKey = new Uint8Array(
            await window.crypto.subtle.exportKey("raw", this.x25519KeyPair.publicKey)
        );
        const x25519PrivateKey = new Uint8Array(
            await window.crypto.subtle.exportKey("pkcs8", this.x25519KeyPair.privateKey)
        );
    ...
    // HKDF key derivation
    private async hkdfKey(salt: Uint8Array, sharedSecret: Uint8Array): Promise<Uint8Array> {
    ...
    private async kyberDecapsulate(ciphertext: Uint8Array): Promise<Uint8Array> {
        // TODO: Integrate liboqs-wasm for actual Kyber768
        // For now, simulate with random bytes
    ...
    async encrypt(
        ...
        receiverKyberPublicKey: Uint8Array
    ): Promise<Uint8Array> {
        ...
        const receiverPublicKey = await window.crypto.subtle.importKey(
            "raw",
            toBufferSource(receiverX25519PublicKey),
            { name: "X25519" },
            false,
            []
        );

        const ssEc = new Uint8Array(
        ...
        const { ciphertext: kyberCt, sharedSecret: ssPq } = await this.kyberEncapsulate(
        ...
        const header = new Uint8Array(
            1 + salt.length + nonce.length + epk.length + kyberCt.length
        );
    ...
    async decrypt(
        ...
        const privateKey = await window.crypto.subtle.importKey(
            "pkcs8",
            toBufferSource(x25519PrivateKey),
            { name: "X25519" },
            false,
            ["deriveBits"]
        );
